...
public class Events implements Listener {
    ...
    public void onDropItem(PlayerDropItemEvent event){
        ...
        if (plugin.getConfig().getBoolean("well.enabled") && player.hasPermission("acuteloot.use-well")){
            new BukkitRunnable() {
                @Override
                public void run() {
                    if(!event.getItemDrop().isValid() || event.getItemDrop() == null){
                        cancel();
                        return;
                    }
                    if(event.getItemDrop().isOnGround() || event.getItemDrop().isInWater()){
                        Location well = new Location(plugin.getServer().getWorld(plugin.getConfig().getString("well.world")), plugin.getConfig().getDouble("well.x"), plugin.getConfig().getDouble("well.y"), plugin.getConfig().getDouble("well.z"));
                        if(event.getItemDrop().getWorld().equals(well.getWorld())){
                            if(event.getItemDrop().getLocation().distance(well) < plugin.getConfig().getDouble("well.max-distance")){
